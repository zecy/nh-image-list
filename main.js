...
const main = () => {
    ...
    const btn = '<button id="getimglist" type="button" class="btn btn-primary" onclick="insertList();">点击获取图片链接</button>';
    ...
    const getImageLinks = () => {
        ...
        let imgLinks = Array.from(rawImgLinks).map(imgLink => {
            ...
            const link = rawLink.replace(linkPattern, "https://i.nhentai.net/galleries/$1/$2.$3");
            return link;
        });
    ...
    const insertList = () => {
        ...
        //// 要插入到这个元素前面
        const hasThumbnailContainer = document.getElementById('thumbnail-container');
